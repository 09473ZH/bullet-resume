...
function formatDataList(dataList: Item[], options: Options = {}): FormattedData[] {
...
<template>
  <div>
    <div id="toolbox">
      <button onclick="window.print()" data-tooltip-target="tooltip-right" data-tooltip-placement="right" type="button"
    ...
    <div id="page" flex flex-wrap px-10 py-5 mx-auto my-8 mb-32 border-1 border-rd-1 overflow-hidden shadow-2xl
         max-w-4xl>
      <!--    header-->
      <header block w-full>
        ...
        <section max-w-full flex pb-5 justify-between>
          ...
          <div flex flex-col>
            <div flex justify-end v-if="tel">
              <span>{{ tel }}</span>
              <span i-carbon-phone font-size-5.5 pl-2></span>
            </div>

            <div flex justify-end v-if="wechat">
              <span>{{ wechat }}</span>
              <span i-carbon-logo-wechat font-size-5.5 pl-2></span>
            </div>

            <div flex justify-end v-if="email">
              <span>{{ email }}</span>
              <span i-carbon-email font-size-5.5 pl-2></span>
            </div>

            <div flex justify-end v-if="github">
              <span>{{ github }}</span>
              <a i-carbon-logo-github font-size-5.5 pl-2 :href="`https://github.com/${github}`" target="_blank"></a>
            </div>
          </div>
